...
正式优化器和独立审查返修接入前的完整金融回测空跑对照、热点见 [组件初验基线](../benchmarks/component_acceptance/REPORT.md)。
该快照默认异步读取、异步推理时，完整回测耗时中位数 **22.338 s**，达到纯数据空跑吞吐率的 **36.08%**。
下文保留的缓存优化与推理延迟实验均为历史占位阶段数据，不能代表当前金融算法的速度。
...
当前 engine 范围包含运行缓存、逐日协议快照、真实金融计算、标签评价与审计收集；历史占位报告不能代表当前性能。
...
按上述方法预热并各测量三次，engine 默认启用异步推理；sync/async 只控制行情读取。
源文件共 158,772,790 字节、1,691,376 行，播放 1,703 个交易日并推理 341 次。
12 次正式运行的交付计数及信号日摘要一致。
...
比例按空跑耗时中位数 / 回测耗时中位数计算；有效带宽按同一批输入文件的逻辑字节数除以完整耗时计算，
不代表物理磁盘或 DRAM 带宽。完整回测包含交易、费用、估值、标签及指标计算；output_dir=None。
标签独立读数的时间计入回测耗时，分子仍固定为播放源文件规模。
...
原版 basic_usage.py 的单次 smoke 为 20.408 s，回调及采样不同，不用于上表比例。
测量明细见 [原始数据](../benchmarks/component_acceptance/data_flow.json)；
该快照的正确性检查、热点与复现命令见 [验收报告](../benchmarks/component_acceptance/REPORT.md)。

## 缓存优化后的空跑带宽比例（历史同步推理基线）

“空跑”使用现有 benchmark 的 data 范围：读取每日行情、构造研究窗口、执行相同的窗口检查/信号摘要及零分推理，
并消费每日价格摘要；不调用运行缓存或金融组件。本节历史报告的 engine 范围运行优化后的占位框架。
...
测试推理函数，不调用金融模块。当前组件验收报告的完整框架范围执行实际市值、收益及事后评价；
历史占位报告仅包含当时的占位调用和审计收集，两者工作量不同。
两者均含测试推理中的窗口检查、信号日摘要和可选模拟延迟。纯数据消费者耗时另记为 consumer_seconds。
文件摘要和预热会使操作系统缓存变热，因此这是本机缓存条件下的数据通路基线，
不是冷盘顺序读取速度；历史占位报告也不代表当前金融算法的运行速度。
